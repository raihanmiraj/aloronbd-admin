...
.then(function () {
 ...
 deleteQuizConfirm = ()=>{
  this.setState({modalshow:false})
  axios.post("/quiz/delete/"+this.state.deleteId)
  .then((response)=>{
    console.log(response.data)
    var allquizdata = this.state.allquizdata;
...
.map(key=>{
 ...
 var image =  "" ;
 try{
  image =  JSON.parse(quizAllMetaData[key].image).thumb;
 }
 catch(e){
 ...
 return (  <tr id={"tableid"+id}>
  <td key={id} class="px-6 py-4 whitespace-nowrap">
...
<span class="statuscolor" aria-hidden="true"  className={statusTogggleClassHandle}></span>
  ...
  <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
    <button  onClick={()=>this.deleteQuizHandler(id)} class="text-indigo-600 hover:text-indigo-900">Delete</button>
  ...
  <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
    ...
    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
     ...
     Cancel
      ...
      <button  onClick={()=>this.deleteQuizConfirm()} class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:ml-3 sm:w-auto sm:text-sm">Accept</button>
